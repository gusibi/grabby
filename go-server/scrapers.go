...
func NewScraper(db *Database, wsManager *WebSocketManager, taskQueue *TaskQueue, logger *zap.Logger, aiEngine *AIEngine) *Scraper {
	return &Scraper{
		db:        db,
		wsManager: wsManager,
		taskQueue: taskQueue,
...
func (s *Scraper) ScrapeRSS(src Source) (int, error) {
	...
	for _, item := range feed.Items {
		...
		if scrapedItem.Content == "" {
			scrapedItem.Content = item.Description // fallback
		}

		if item.PublishedParsed != nil {
			scrapedItem.PublishedAt = item.PublishedParsed
		} else {
			now := time.Now()
...
func (s *Scraper) ScrapeAPI(src Source) (int, error) {
...
func (s *Scraper) ScrapeWeb(src Source, logID int64) (int, error) {
	...
	if config.ListSelector != "" {
		...
		resp, err := s.wsManager.SendMessage(ctx, &BrowserRequest{
		...
		for _, link := range allLinks {
			...
			var exists bool
			err := s.db.db.QueryRow("SELECT EXISTS(SELECT 1 FROM scraped_items WHERE url = ?)", link).Scan(&exists)
			if err != nil {
				continue
			}
			if !exists {
		...
		var exists bool
		err := s.db.db.QueryRow("SELECT EXISTS(SELECT 1 FROM scraped_items WHERE url = ?)", src.URL).Scan(&exists)
		if err == nil && !exists {
	...
	if len(urlsToScrape) == 0 {
		...
		// Log as skipped
		s.db.db.Exec("UPDATE fetch_logs SET status = 'skipped', finished_at = CURRENT_TIMESTAMP WHERE id = ?", logID)
		return 0, nil
	}
