...
type ProfileSelector struct {
	...
	strategy     string
	activeID     string
	profiles     []domainai.AIProviderProfile // enabled, sorted by priority
...
func NewProfileSelector(settings domainai.AISettings) *ProfileSelector {
	enabled := make([]domainai.AIProviderProfile, 0)
...
func (s *ProfileSelector) Next() *domainai.AIProviderProfile {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ProfileSelector) findActive() *domainai.AIProviderProfile {
...
func (s *ProfileSelector) nextRoundRobin() *domainai.AIProviderProfile {
	n := len(s.profiles)
	if n == 0 {
		return nil
	}
...
func (s *ProfileSelector) nextFailover() *domainai.AIProviderProfile {
