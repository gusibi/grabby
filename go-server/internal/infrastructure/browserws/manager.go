...
type WebSocketManager struct {
	...
	pendingResponses  map[string]chan *capture.BrowserResponse
...
func NewWebSocketManager(logger *zap.Logger) *WebSocketManager {
	return &WebSocketManager{
		...
		pendingResponses:  make(map[string]chan *capture.BrowserResponse),
...
func (wm *WebSocketManager) GetBrowserList() []browser.BrowserInfo {
	wm.mu.RLock()
	defer wm.mu.RUnlock()
...
func (wm *WebSocketManager) ResolveBrowserConnID(name string) (string, error) {
	...
	if name == "" {
		defaultName := config.GetSettings().DefaultBrowser
		if defaultName != "" {
...
func (wm *WebSocketManager) SendMessage(ctx context.Context, req *capture.BrowserRequest, targetConnID string) (*capture.BrowserResponse, error) {
	...
	respCh := make(chan *capture.BrowserResponse, 1)
	wm.mu.Lock()
	...
	timeout := time.Duration(config.GetSettings().WebsocketTimeout * float64(time.Second))
...
func (wm *WebSocketManager) HandleResponse(resp *capture.BrowserResponse) {
	if resp.MessageID == "" {
