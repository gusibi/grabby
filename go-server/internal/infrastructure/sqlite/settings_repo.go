...
func (d *Database) LoadAISettings(env AISettings) (AISettings, error) {
	...
	if !strings.Contains(s.DailyPrompt, "12 条到 20 条") && strings.Contains(s.DailyPrompt, "挑选 2-3 篇最重磅内容") {
		s.DailyPrompt = DefaultDailyPrompt
		_ = d.SaveSetting("ai_daily_prompt", DefaultDailyPrompt)
	}
